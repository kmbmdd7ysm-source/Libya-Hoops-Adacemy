...
const VERSION='lha-v6-20260728-images',STATIC=`${VERSION}-static`,MEDIA=`${VERSION}-media`,PAGES=`${VERSION}-pages`;
const CORE=['/','/offline','/favicon.svg','/site.webmanifest','/brand/lha-mark-black.png','/brand/lha-mark-white.png','/brand/lha-wordmark-black.svg','/brand/lha-wordmark-white.svg','/media/hero/lha-hero-poster.svg'];
...
self.addEventListener('fetch',e=>{const r=e.request,u=new URL(r.url);if(r.method!=='GET'||PRIVATE.test(u.href)||u.origin!==location.origin)return;
 ...
 if(/\.(?:png|jpg|jpeg|webp|avif|svg|woff2?)$/i.test(u.pathname)){e.respondWith((async()=>{const c=await caches.open(MEDIA);try{const res=await fetch(r,{cache:'no-store'});if(res.ok){c.put(r,res.clone());limit(MEDIA,160)}return res}catch{return await c.match(r,{ignoreVary:true})||Response.error()}})());return}
