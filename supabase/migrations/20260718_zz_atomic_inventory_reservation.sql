...
begin
  ...
  if not v_has_physical then
    ...
    v_shipping_total := case when v_subtotal >= 70.00 then 0 else round(20.00 / v_usd_to_lyd_rate, 2) end;
    v_shipping_summary := coalesce(p_shipping,'{}'::jsonb) || jsonb_build_object(
      'rate_amount',20.00,
      'rate_currency','LYD',
      'canonical_shipping_total',v_shipping_total,
      'canonical_currency','USD',
      'free_shipping_threshold_amount',70.00,
  ...
  v_order_number := 'LHA-' || to_char(clock_timestamp(),'YYYYMMDD') || '-' || lpad(nextval('public.order_number_seq')::text, 7, '0');
