      ...
      <section className="section favorites-page">
        <div className="container">
          ...
          <div className="page-title-row">
            ...
            <span aria-live="polite">{saved.length}</span>
          </div>
          {(userData?.status === 'error' || userData?.status === 'offline') && (
            <div className="sync-warning" role="status" aria-live="polite">
              <div>
                ...
                <p>
                  {pick({
                    en: 'Your locally saved products are still shown below.',
                    ar: 'لا تزال منتجاتك المحفوظة محليًا ظاهرة أدناه.',
                  })}
                </p>
              </div>
              <button
          ...
          {userData?.status === 'syncing' && !saved.length ? (
            <div className="empty-state" role="status">
              <h2>
                {pick({ en: 'Loading saved products…', ar: 'جارٍ تحميل المنتجات المحفوظة…' })}
              </h2>
            </div>
          ) : saved.length ? (
            ...
            <div className="empty-state">
              <h2>{pick({ en: 'No saved products yet', ar: 'لا توجد منتجات محفوظة بعد' })}</h2>
              <p>
                {pick({
