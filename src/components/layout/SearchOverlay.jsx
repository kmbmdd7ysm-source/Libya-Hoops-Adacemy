...
export default function SearchOverlay({ open, onClose, triggerRef }) {
  ...
  const suggestions = useMemo(() => getSearchSuggestions(query, 7), [query]);
  ...
  const close = (source = 'cancel') => {
    ...
    if (historyManaged.current && window.history.state?.lhaSearchOverlay) {
      closingFromPop.current = true;
      onClose();
      window.history.back();
    } else {
  ...
  useEffect(() => {
    ...
    if (!window.history.state?.lhaSearchOverlay) {
      window.history.pushState(
        { ...(window.history.state || {}), lhaSearchOverlay: true },
        '',
    ...
    return () => {
      ...
      if (!closingFromPop.current && window.history.state?.lhaSearchOverlay)
        window.history.replaceState(
          { ...window.history.state, lhaSearchOverlay: undefined },
          '',
          location.pathname + location.search + location.hash,
        );
    ...
    >
      <div className="search-overlay-inner">
        <h1 id={`${listId}-title`} className="sr-only">
          {pick({ en: 'Search Libya Hoops Academy', ar: 'البحث في أكاديمية ليبيا هوبس' })}
        </h1>
