...
const VERSION='lha-v4-20260716',STATIC=`${VERSION}-static`,MEDIA=`${VERSION}-media`,PAGES=`${VERSION}-pages`;
...
self.addEventListener('fetch',e=>{const r=e.request,u=new URL(r.url);if(r.method!=='GET'||PRIVATE.test(u.href)||u.origin!==location.origin)return;
 ...
 if(/\.(?:png|jpg|jpeg|webp|avif|svg|woff2?)$/i.test(u.pathname)){e.respondWith((async()=>{const c=await caches.open(MEDIA),hit=await c.match(r);const update=fetch(r).then(res=>{if(res.ok){c.put(r,res.clone());limit(MEDIA,80)}return res}).catch(()=>null);return hit||await update||Response.error()})());return}
