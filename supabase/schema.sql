...
create table if not exists public.profiles(id uuid primary key references auth.users on delete cascade,display_name text check(char_length(display_name)<=80),first_name text,last_name text,preferred_language text default 'en',preferred_size text,preferred_colors text[] default '{}',preferred_categories text[] default '{}',marketing_consent boolean default false,created_at timestamptz default now(),updated_at timestamptz default now());
