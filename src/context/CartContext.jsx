    ...
    case 'ADD': {
      ...
      return [
        ...state,
        {
          ...item,
          quantity:
            item.type === 'product' ? Math.min(item.quantity, item.maxStock ?? Infinity) : 1,
          updatedAt: new Date().toISOString(),
        },
      ];
    }
    case 'UPDATE_QTY':
      return state.map((i) =>
        i.key !== action.key
          ...
          : {
              ...i,
              quantity: Math.max(1, Math.min(action.quantity, i.maxStock ?? Infinity)),
              updatedAt: new Date().toISOString(),
            },
      );
    case 'REMOVE':
...
export function CartProvider({ children }) {
  ...
  useEffect(() => {
    ...
    channel.current = createChannel('lha-cart', (msg) => {
